    ...
    void UpdateClosestTarget()
    {
        if (VuforiaRuntimeUtilities.IsVuforiaEnabled() && VuforiaARController.Instance.HasStarted)
        {
                ...
                this.currentVuMark != this.closestVuMark)
            {
                ...
                //StartCoroutine(ShowPanelAfter(0.5f, vuMarkId, vuMarkDataType, vuMarkDesc, vuMarkImage, isGold));
            }
        }
    }

    IEnumerator ShowPanelAfter(float seconds, string vuMarkId, string vuMarkDataType, string vuMarkDesc, Sprite vuMarkImage, bool isGold)
    {
        yield return new WaitForSeconds(seconds);

        if (this.nearestVuMarkScreenPanel)
        {
            nearestVuMarkScreenPanel.Show(vuMarkId, vuMarkDataType, vuMarkDesc, vuMarkImage, isGold);
        }
    }
